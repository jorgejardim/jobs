...
* @return String[]  
*/
function somadata($pData, $pDias, $converte=1) { 
...
* @return String[]  
*/
function subtraidata($pData, $pDias, $converte=1) { 
...
* @return Int[]   Quantidade de dias 
*/
function diasUteis($dataInicial, $dataFinal, $feriados) {

	$total_feriados = sizeof($feriados);
    $qtd_dias       = diferencas_de_datas($dataInicial,$dataFinal);
    $dias_uteis     = 0;
    $dias_nao_uteis = 0;
    ...
    for($i=0; $i<=$qtd_dias; $i++) {
	    ...
	    if($i>0) {
	  	    $data_dia = somadata(converte_data($dataInicial),$i);
        } else {
...
* @return Int[]
*/
function dataUtil($data, $feriados) {
...
*/
function datasUteis($dataInicial, $dataFinal, $feriados) {
...
* @return String[]  
*/
function subtraiDatasUteis($dataInicial, $qtdDias, $feriados) {

	$i = 0;
	do {
		if($i>0) {
			$dataFinal = subtraidata(converte_data($res[0]), 1, 1);
		} else {
			$dataFinal = subtraidata(converte_data($dataInicial), 1, 1);
		}
		do {
			if($i>0) {
				$datas = datasUteis($dataFinal, $res[0], $feriados);
			} else {
				$datas = datasUteis($dataFinal, $dataInicial, $feriados);
			}
			$dataFinal = subtraidata(converte_data($dataFinal), 1, 1);
...
* @return Array[] Datas uteis (aaaa-mm-dd)
*/
function proxDiaUtil($data, $feriados) {

	do {
		$data  = somadata($data, 1, 0);
		$total = diasUteis(converte_data($data), converte_data($data), $feriados);
...
* @return Array[] Datas uteis (aaaa-mm-dd)
*/
function diaUtilAnterior($data, $feriados) {

	do {
		$data  = subtraidata($data, 1, 0);
		$total = diasUteis(converte_data($data), converte_data($data), $feriados);
...
* @return Array[] Datas uteis (aaaa-mm-dd)
*/
function datasUteisMes($mes, $ano, $feriados) {
    ...
    return datasUteis($dataInicial, $dataFinal, $feriados);
}
...
function datasMes($mes, $ano) {
    ...
    return diasEntreDatas($dataInicial, $dataFinal);
}
...
* @return String
*/
function diaSemanaData($data,$abreviado=0) {  
...
function datasDaSemana($data) {
	 ...
	 if($dia==0) {
	     $data = somadata(converte_data($data), 1); //muda para segunda
	 } elseif($dia==6) {
	     $data = subtraidata(converte_data($data), 1); //muda para sexta
	 } 
	 if($dia==1) {
	     $inicio = $data;
	 } elseif($dia>1) {
		 $inicio = subtraidata(converte_data($data), $dia-1);	 
	 }
	 $fim = somadata(converte_data($inicio), 4);
	 
	 return diasEntreDatas($inicio, $fim);
}
