...
class MoIP
{
    ...
    public function setComissoes($comissoes)
    {
    ...
    public function getXML()
    {
        ...
        //$boleto = $this->xml->InstrucaoUnica->addChild('Boleto');
        //$boleto->addChild('DataVencimento','24/04/2011');
        ...
        if(!empty($this->pagador))
        {
            ...
            (isset($p['logradouro']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Logradouro' , $this->pagador['endereco']['logradouro']):null;
            
            (isset($p['numero']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Numero' , $this->pagador['endereco']['numero']):null;

            (isset($p['complemento']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Complemento' , $this->pagador['endereco']['complemento']):null;

            (isset($p['bairro']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Bairro' , $this->pagador['endereco']['bairro']):null;

            (isset($p['cidade']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Cidade' , $this->pagador['endereco']['cidade']):null;

            (isset($p['estado']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Estado' , $this->pagador['endereco']['estado']):null;

            (isset($p['pais']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'Pais' , $this->pagador['endereco']['pais']):null;

            (isset($p['cep']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'CEP' , $this->pagador['endereco']['cep']):null;

            (isset($p['telefone']))?$this->xml->InstrucaoUnica->Pagador->EnderecoCobranca->addChild( 'TelefoneFixo' , $this->pagador['endereco']['telefone']):null;

        }

        if(!empty($this->recebedor))
        {
            $r = $this->recebedor;
            $this->xml->InstrucaoUnica->addChild('Recebedor');
            (isset($r['login']))?$this->xml->InstrucaoUnica->Recebedor->addChild( 'LoginMoIP' , $this->recebedor['login'] ):null;
            (isset($r['email']))?$this->xml->InstrucaoUnica->Recebedor->addChild( 'Email' , $this->recebedor['email']):null;
        }
        ...
        if(!empty($this->comissoes))
        {
            $c = $this->comissoes;
            $this->xml->InstrucaoUnica->addChild('Comissoes');
            foreach(array_keys($this->comissoes) as $k) {

                $comissionamento = $this->xml->InstrucaoUnica->Comissoes->addChild('Comissionamento');
                $comissionamento->addChild('Razao', 'Teste');
                $comissionamento->addChild('Comissionado');
                $comissionamento->Comissionado->addChild('LoginMoIP', $this->comissoes[$k]['login']);
                $comissionamento->addChild('ValorPercentual', $this->comissoes[$k]['percentual']);
            }
        }
    ...
    public function getResposta()
    {
        ...
        if ((!empty($this->resposta->erro))||(!empty($xml->Resposta->Erro))) {
            return (object) array('sucesso'=>false,'erro'=>$this->resposta->erro .''. $xml->Resposta->Erro);
        }
        ...
        $return->erro    = (string)$xml->Resposta->Erro;
