...
class UsersController extends AppController {
        ...
        if(!empty($data)) { 
            ...
            if(!$existe) {
                ...
                if ($this->User->save($data)) {
                    ...
                    $this->Session->setFlash('Erro: O cadastro não foi salvo. Por favor, tente novamente.');
                    return false;
                }            
    ...
    function register() {
        ...
        $validate = array(
            'password_confirmation' => array(  
                'match' => array(  
                    ...
                    'required'      => true,  
                    'allowEmpty'    => false,  
                    'message'       => __('Deve ser igual ao campo Senha.', true)  
                )  
            )         
        ); 
        $this->User->validate = array_merge($this->User->validate, $validate);
        ...
        $this->loadModel('UsersAddress');
        $this->UsersAddress->validate = $validate;
        
        if (!empty($this->data)) {
            
            //validacoes
            $this->User->set($this->data);  
            $this->UsersData->set($this->data);
            $this->UsersAddress->set($this->data);
            
            if($this->User->validates() && $this->UsersData->validates() && $this->UsersAddress->validates() ) {
