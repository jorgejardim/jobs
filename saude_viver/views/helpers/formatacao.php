 ...
 */
	function moedaPorExtenso($numero) {
        ...
        function dataCurso($data_inicial, $data_final) {

            $exp = explode(' ', $data_inicial);
            $data_inicial = $exp[0];
            $hora_inicial = substr($exp[1],0,5);
            $dia_inicial  = substr($exp[0],8,2);
            $mes_inicial  = $this->escreve_mes(substr($exp[0],5,2));
            
            $exp = explode(' ', $data_final);
            $data_final = $exp[0];
            $hora_final = substr($exp[1],0,5);            
            $dia_final  = substr($exp[0],8,2);
            $mes_final  = $this->escreve_mes(substr($exp[0],5,2));            
            
            if($data_inicial==$data_final) {
            
                $res  = $dia_inicial.' de '.$mes_inicial.' ';
                $res .= '('.$this->diaSemana($data_inicial).') ';
                $res .= '- das '.str_replace(':','h',$hora_inicial);
                $res .= ' às '.str_replace(':','h',$hora_final);
            
            } else {
            
                $res  = $dia_inicial.' de '.$mes_inicial.' ';
                $res .= '('.$this->diaSemana($data_inicial).') ';
                $res .= 'as '.str_replace(':','h',$hora_inicial);             
